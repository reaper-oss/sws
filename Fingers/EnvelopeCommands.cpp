...
{
    TrackEnvelope *trackEnv = GetSelectedEnvelope(NULL);
    int numAutoItems = CountAutomationItems(trackEnv);
    for (int autoItem = -1; autoItem < numAutoItems; autoItem++)
    {
        BR_Envelope envelope(trackEnv, true, autoItem);
